...
async function getOrCreateActor(name: string) {
  let actor = await prisma.crew.findFirst({
    where: {
      name,
      role: 'ACTOR',
    },
  ...
  if (!actor) {
    actor = await prisma.crew.create({
      data: {
        name,
        role: 'ACTOR',
      },
...
async function getOrCreateDirector(name: string) {
  let director = await prisma.crew.findFirst({
    where: { name, role: 'DIRECTOR' },
  }) // the role: ACTOR  or DIRECTOR
  if (!director) {
    director = await prisma.crew.create({
      data: {
        name,
        role: 'DIRECTOR',
      },
...
async function main() {
  ...
  for (const m of moviesList.slice(0, 20)) {
    const detailsRes = await fetch(`${TMDB_BASE}/movie/${m.id}?api_key=${TMDB_API_KEY}`)
    const details = await detailsRes.json()

    const creditsRes = await fetch(`${TMDB_BASE}/movie/${m.id}/credits?api_key=${TMDB_API_KEY}`)
    const credits = await creditsRes.json()

    // Extract top actors and director
    const topActors = credits.cast?.slice(0, 3) ?? []
    const director = credits.crew?.find((c: { job: string; name: string }) => c.job === 'Director')
    ...
    const directorRecord = director ? await getOrCreateDirector(director.name) : null
    ...
    await prisma.movie.create({
      data: {
        ...
        directors: directorRecord ? { connect: [{ id: directorRecord.id }] } : undefined,
      },
    })
